...
export default function AgentChat() {
  ...
  // Dedup guard: track the last N message fingerprints to discard exact duplicates
  // that might arrive when stale socket handlers fire during a reconnect cycle.
  ...
  useEffect(() => {
    ...
    ws.onMessage = (msg: WsMessage) => {
      switch (msg.type) {
        ...
        case 'done': {
          ...
          if (content) {
            // Dedup: skip if we've seen this exact content within the last 2 seconds
            const fingerprint = `${msg.type}:${content}`;
            if (!recentFingerprintsRef.current.has(fingerprint)) {
              recentFingerprintsRef.current.add(fingerprint);
              setTimeout(() => recentFingerprintsRef.current.delete(fingerprint), 2000);
              setMessages((prev) => [
                ...prev,
                {
    ...
    return () => {
      // Null out handlers before disconnect so no stale callbacks fire
      ws.onMessage = null;
      ws.onOpen = null;
      ws.onClose = null;
      ws.onError = null;
